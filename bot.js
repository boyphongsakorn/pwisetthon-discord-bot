        ...
        .then((json) => {
            ...
            if (json.length == 7 || json.length == 8 || json.length == 9) {
                if (json[0][1] == "0" || json[0][1] == 0 || json[0][1] == "xxxxxx" || json[0][1] == "XXXXXX") {
                    ...
                    try {
                        fileContents = fs.readFileSync('check.log');
                    } catch (err) {
                    ...
                    if (fileContents) {
                        if (fileContents != '0') {
                            fs.writeFile('check.log', '0', function (err) {
                                if (err) {
                                    throw err
                                };
                                console.log('Saved!');
                    ...
                    /*fs.readFile('check.log', function(err, data) {
                        if(data != "0"){
                            fs.writeFile('check.log', '0', function (err) {
                                if (err){
                                    throw err
                    ...
                    if (fileContents) {
                        if (fileContents != "1") {
                            fs.writeFile('check.log', '1', function (err) {
                                if (err) {
                                    throw err
                                };
                                console.log('Saved!');
